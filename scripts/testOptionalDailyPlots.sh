...
run_case() {
    ...
    cat > "$scripts_dir/GetPlotOptions.py" <<PY
#!/usr/bin/env python3
print("$plot_hdz $plot_bi true north")
PY
    chmod +x "$scripts_dir/GetPlotOptions.py"
    ...
    local upload_stub="$TEST_BASE_PATH/scripts/uploadRemote.sh"
...
UPLOAD
    ...
    if [ "$exit_code" -ne "$expected_exit" ]; then
        ...
        tail -n 20 "$TEST_BASE_PATH/logfiles/log-Magnetometer.txt" 2>/dev/null || true
