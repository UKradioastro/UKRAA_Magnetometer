...
usage() {
    ...
    echo "  --archive   also write plots/day/Activity/YYYY/YYYY-MM/YYYY-MM-DD.png (default is temp only)"
}
...
if [ "$ARCHIVE_MODE" -eq 1 ]; then
    echo "Archive: $BASE_PATH/plots/day/Activity/$(date -d "$TARGET_DATE" +%Y)/$(date -d "$TARGET_DATE" +%Y-%m)/$TARGET_DATE.png"
fi
