...
if MAGNETOMETER_BASE_PATH="$BASE_PATH" su pi -c "/usr/bin/python3 $BASE_PATH/scripts/ProcessDailySummaryACM0.py >> $MAIN_LOG 2>> $ERROR_LOG"; then
...
if MAGNETOMETER_BASE_PATH="$BASE_PATH" MAGNETOMETER_PLOT_PERIOD=day su pi -c "/usr/bin/gnuplot $BASE_PATH/scripts/PlotPeriodXYZACM0.gp >> $MAIN_LOG 2>> $ERROR_LOG"; then
...
if [ "$PLOT_HDZ" = "true" ]; then
    if MAGNETOMETER_BASE_PATH="$BASE_PATH" MAGNETOMETER_PLOT_PERIOD=day su pi -c "/usr/bin/gnuplot $BASE_PATH/scripts/PlotPeriodHDZACM0.gp >> $MAIN_LOG 2>> $ERROR_LOG"; then
...
if [ "$PLOT_BI" = "true" ]; then
    if MAGNETOMETER_BASE_PATH="$BASE_PATH" MAGNETOMETER_PLOT_PERIOD=day su pi -c "/usr/bin/gnuplot $BASE_PATH/scripts/PlotPeriodBIACM0.gp >> $MAIN_LOG 2>> $ERROR_LOG"; then
...
if MAGNETOMETER_BASE_PATH="$BASE_PATH" /bin/bash "$BASE_PATH/scripts/processPeriodPlotsACM0.sh"; then
