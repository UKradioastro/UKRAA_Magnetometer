...
if (is_missing == 1) {
    print system("date +'%Y-%m-%d %H:%M:%S'") \
          ." : PlotDataHDZ.gp        : FAILED - data file missing"
    exit
}
...
if (dirCreated == 1) {
    print system("date +'%Y-%m-%d %H:%M:%S'") \
          ." : PlotDataHDZ.gp        : New directory created : ".plotMonthDir
}
...
print system("date +'%Y-%m-%d %H:%M:%S'") \
      ." : PlotDataHDZ.gp        : Started HDZ plot for " \
      .dateTag
...
print system("date +'%Y-%m-%d %H:%M:%S'") \
      ." : PlotDataHDZ.gp        : Completed HDZ plot for " \
      .dateTag
