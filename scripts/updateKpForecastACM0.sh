...
if ! DOWNLOAD_OUTPUT=$(/usr/bin/python3 "$BASE_PATH/scripts/UpdateKpForecastACM0.py" "$KP_URL" "$KP_DATA" 2>&1); then
    log_msg "updateKpForecastACM0.sh    : FAILED - $DOWNLOAD_OUTPUT" >> "$ERROR_LOG"
    exit 1
fi

if MAGNETOMETER_BASE_PATH="$BASE_PATH" /usr/bin/gnuplot "$BASE_PATH/scripts/PlotKpForecastACM0.gp" >> "$MAIN_LOG" 2>> "$ERROR_LOG"; then
