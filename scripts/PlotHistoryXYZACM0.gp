...
if (is_missing == 1) {
    print system("date +'%Y-%m-%d %H:%M:%S'") \
          ." : PlotHistoryXYZACM0.gp    : FAILED - data file missing for window " \
          .windowKey
    exit
}

dateTag = system("date -d yesterday +'%Y-%m-%d'")
archivePlot = basePath."/plots/history/".windowKey."/".dateTag."_XYZ_plot.png"
tempPlot = basePath."/temp/history/".windowKey."/XYZ.png"
...
stats fileData using 5 output prefix "XSTAT" nooutput
stats fileData using 6 output prefix "YSTAT" nooutput
stats fileData using 7 output prefix "ZSTAT" nooutput
...
plotTitle = sprintf("XYZ magnetic field history (%s)\n%s UTC to %s UTC", windowKey, startX, endX)

set xrange [startX:endX]

print system("date +'%Y-%m-%d %H:%M:%S'") \
      ." : PlotHistoryXYZACM0.gp    : Started XYZ history plot for window " \
      .windowKey
...
print system("date +'%Y-%m-%d %H:%M:%S'") \
      ." : PlotHistoryXYZACM0.gp    : Completed XYZ history plot for window " \
      .windowKey
