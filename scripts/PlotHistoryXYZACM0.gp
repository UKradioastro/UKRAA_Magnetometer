...
archivePlot = archiveDir."/".system("sh -lc 'if [ -n \"${MAGNETOMETER_TARGET_DATE:-}\" ]; then printf %s \"$MAGNETOMETER_TARGET_DATE\"; else date -d yesterday +%Y-%m-%d; fi'")."_XYZ.png"
tempDir = basePath."/temp/history/".windowName
tempPlot = tempDir."/XYZ.png"
...
plotTitle = sprintf("XYZ magnetic field for last %s\n%s UTC to %s UTC", windowName, startX, endX)
