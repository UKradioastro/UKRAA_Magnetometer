...
if (is_missing == 1) {
    print system("date +'%Y-%m-%d %H:%M:%S'") \
          ." : PlotHistoryHDZACM0.gp    : FAILED - data file missing for window " \
          .windowKey
    exit
}

dateTag = system("date -d yesterday +'%Y-%m-%d'")
archivePlot = basePath."/plots/history/".windowKey."/".dateTag."_HDZ_plot.png"
tempPlot = basePath."/temp/history/".windowKey."/HDZ.png"
...
plotTitle = sprintf("HDZ magnetic field history (%s)\n%s UTC to %s UTC", windowKey, startX, endX)

set xrange [startX:endX]

print system("date +'%Y-%m-%d %H:%M:%S'") \
      ." : PlotHistoryHDZACM0.gp    : Started HDZ history plot for window " \
      .windowKey
...
unset xlabel
set yrange [HSTAT_min - 200:HSTAT_max + 200]
...
print system("date +'%Y-%m-%d %H:%M:%S'") \
      ." : PlotHistoryHDZACM0.gp    : Completed HDZ history plot for window " \
      .windowKey
