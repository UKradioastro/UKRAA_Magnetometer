...
if ! PLOT_KP=$(MAGNETOMETER_BASE_PATH="$BASE_PATH" /usr/bin/python3 "$BASE_PATH/scripts/GetKpOptions.py" 2>&1); then
    log_msg "updateKpForecast.sh    : FAILED to read Kp plot option: $PLOT_KP" >> "$ERROR_LOG"
    exit 1
fi

if [ "$PLOT_KP" != "true" ] && [ "$PLOT_KP" != "false" ]; then
    log_msg "updateKpForecast.sh    : FAILED - unexpected Kp plot option: '$PLOT_KP'" >> "$ERROR_LOG"
    exit 1
fi

if [ "$PLOT_KP" = "false" ]; then
    rm -f "$KP_DATA" "$KP_TEMP"
    log_msg "updateKpForecast.sh    : Planetary Kp forecast disabled (plot_kp=false)" >> "$MAIN_LOG"
    exit 0
fi
...
log_msg "updateKpForecast.sh    : FAILED rendering planetary Kp forecast" >> "$ERROR_LOG"
exit 1