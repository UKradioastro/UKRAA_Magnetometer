...
# Required history web files check (XYZ history is unconditional).
for history_window in 7d 1m 3m 6m 1y; do
    if [ ! -f "$WEB_TEMP_DIR/history/$history_window/XYZ.png" ]; then
        status="FAIL"
        append_issue "missing web history file: $WEB_TEMP_DIR/history/$history_window/XYZ.png"
    fi
done

# Optional daily and history web files are only required when enabled.
